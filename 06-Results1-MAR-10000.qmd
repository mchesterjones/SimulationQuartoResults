---
title: " Results for MAR, N=10,000"
---
...
```{r}
#| label: open_datasets_mar_n10000 
#| include: false
...
### Predictive Performance from Study 1 N=10,000 under Missing at Random {#-predictive-performance-mar-10000}

#### Brier Score

At 1% outcome prevalence, there was almost no difference in the brier score for different imputation methods. At higher prevalence (5% and 10%), the CCA had lower (more accurate) brier scores although the confidence intervals overlapped. The range of all brier scores was small with the lowest average for CCA at 1% prevalence and 75% missingness (0.0082, 95% CI 0.0048, 0.0118) and the highest average for MI without the outcome at 10% prevalence and 75% missingness (0.085, 95% CI 0.079, 0.090).

```{r #fig-brier}
#| label: brier_score
#| echo: false
#| fig-cap: Brier Score for each combination of prevalence at n=10,000 under Missing at Random
#| fig-width: 10
#| fig-height: 12 
...
ggplot(simulation_parameters_long %>% filter(Measure=="Brier Score"), 
  ...
  geom_point(data = combined_df, aes(x = Brier, y = Method), 
             shape =4, position = position_jitter(width = 0.003), alpha = 0.5) + 
    labs(y = NULL,
         x = NULL,
         colour = "Method\n(Mean, 95% CI)") +
    theme_minimal() + 
